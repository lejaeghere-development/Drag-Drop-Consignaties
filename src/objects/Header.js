...
export default class Header extends Phaser.GameObjects.Group {
    constructor(game) {
        ...
        if(!Global.headerTriggered){
            ...
            document.querySelector("#change_address_confirm #confirm").addEventListener("click", (v)=>{
                v.preventDefault();
                v.stopImmediatePropagation();
                this.doChangeAddress();
            });
            document.querySelector("#change_address_confirm #cancel").addEventListener("click", (v) => {
                v.preventDefault();
                v.stopImmediatePropagation();
            ...
            //     this.skipGame();
            // });
            document.querySelector("#skip_confirm #cancel").addEventListener("click", (v) => {
                v.preventDefault();
                v.stopImmediatePropagation();
                this.cancelSkip();
            });
           
            document.querySelector("#logout_confirm #lg_confirm").addEventListener("click", (v) => {
                v.preventDefault();
                v.stopImmediatePropagation();
                this.doLogout();
            });
            document.querySelector("#logout_confirm #lg_cancel").addEventListener("click", (v) => {
                v.preventDefault();
                v.stopImmediatePropagation();
                this.hideLogout();
            });
            ...
            document.querySelector("#ready_info #confirm").addEventListener("click", (v) => {
                v.preventDefault();
                v.stopImmediatePropagation();
                this.skipGame();
            });
            document.querySelector("#ready_info #cancel").addEventListener("click", (v) => {
                v.preventDefault();
                v.stopImmediatePropagation();
    ...
    setUp() {
        ...
        Global.emitter.on('header:update_crate_status', this.updateCrateBtnStatus.bind(this));
        Global.emitter.on('header:update_skip_frame', this.updateSkipFrame.bind(this));
        ...
        Global.emitter.on('trigger_change_address', this.onChangeAddressPress.bind(this));
    ...
    init() {
        ...
        this.crateCreatedTxt = this.scene.make.text({
                ...
                style: {
                    font: (Global.isMobile) ? '' + String(45 * this.scaleFact) + 'px greycliff-bold' : '' + String(45 * this.scaleFact) + 'px greycliff-bold',
                    fill: '#ffffff',
                    align: "center"
                }
            })
        ...
        this.toggleSwitchLabel = this.scene.make.text({
            ...
            style: {
                font: (Global.isMobile) ? '' + String(55 * this.scaleFact) + 'px greycliff-bold' : '' + String(55 * this.scaleFact) + 'px greycliff-bold',
                fill: '#ffffff',
                align: "center"
            }
        })
        ...
        this.toggleInfoIcon= this.create(this.crateCreatedBG.x + 1320 * this.scaleFact, this.crateCreatedBG.y - (40) * this.scaleFact, 'items','infoBtn0000')
        .setScale(this.scaleFact*0.75)
        .setInteractive({
            cursor: 'pointer'
        })
        ...
        // .setReadOnly();
        this.toggleSwitch.on('valuechange', function(value) {
            // value: checked
        ...
        if(window.isLoggedIn){
            this.changeAddress = this.create(this.c_w-this.extraLeftPer - 1400*this.scaleFact, 150 * this.scaleFact+this.extraTop, 'items', 'changeAddress0000')
            .setScale(this.scaleFact * 0.75)
            ...
            .on('pointerover', this.onHover.bind(this, 'changeAddress', 'changeAddress_10000'))
            .on('pointerout', this.onHover.bind(this, 'changeAddress', 'changeAddress0000'))
        }
   
        this.skipBtn = this.create(this.c_w-this.extraLeftPer - ((window.isLoggedIn)?(Object.keys(window.address).length>1?(1200/* 1830 */):1400):800)*this.scaleFact, 150 * this.scaleFact+this.extraTop, 'items', 'skip0000')
        .setScale(this.scaleFact * 0.75)
        .setInteractive({
            cursor: 'pointer'
        })
        ...
        this.skipHintBG= this.create(this.skipBtn.x-330*this.scaleFact, this.skipBtn.y+340*this.scaleFact, 'items', 'hintBG0000')
        ...
        this.menuTrigger= this.create(-300*this.scaleFact, -300*this.scaleFact, 'items', 'menu_trigger0000')
        .setScale(this.scaleFact * 1)
        .setInteractive({
            cursor: 'pointer'
        })
        ...
        this.skipHintTxt = this.scene.make.text({
            ...
            style: {
                font: (Global.isMobile) ? '' + String(55 * this.scaleFact) + 'px greycliff-medium' : '' + String(55 * this.scaleFact) + 'px greycliff-medium',
                fill: '#055A38',
                align: "left",
        ...
        this.hideCheck= this.create(this.skipHintBG.x-475*this.scaleFact, this.skipHintBG.y+150*this.scaleFact, 'items', 'hint_check0000')
        ...
        this.hideInfo = this.scene.make.text({
            ...
            style: {
                font: (Global.isMobile) ? '' + String(40 * this.scaleFact) + 'px greycliff-medium' : '' + String(40 * this.scaleFact) + 'px greycliff-medium',
                fill: '#055A38',
                align: "left",
                wordWrap: { width: 1000*this.scaleFact }
            }
        ...
        this.hideBtn= this.create(this.skipHintBG.x+475*this.scaleFact, this.skipHintBG.y+150*this.scaleFact, 'items', 'hint_close0000')
        .setScale(0)
        .setAlpha(0)
        .setOrigin(1,0.5)
        .setInteractive({
    ...
    updateHideChecked(){
        ...
        this.hideCheck && this.hideCheck.setFrame(!this.hideChecked?'hint_check0000':'hint_check_active0000')

        if(this.hideChecked){
            document.querySelector(".mobile_skip_ui .mobile_skip_check :nth-child(2)").classList.add('active');
        }else{
            document.querySelector(".mobile_skip_ui .mobile_skip_check :nth-child(2)").classList.remove('active');
        }
    }
    updateToggleState(checkBeforeToggle,status){
  
        if(!checkBeforeToggle || ( checkBeforeToggle && Global.canUseCustomToggle)){
            this.toggleSwitch.setReadOnly(status);
            this.toggleSwitch.setAlpha(status?.1:1)
    ...
    toggleType(value, isStart= false, quick=false){
        ...
        if(!isStart){
            Global.emitter.emit('crate:select', 24, (Global.isToggleOn?'custom':'fixed'), true);
            Global.emitter.emit('bottle:remove');
        }
        // this.scene.tweens.add({
        //     targets: this.toggleBG2,
    ...
    updateCrateBtnStatus(status, isPrefill){
        // alert(status);
        if(!this.skipInfoShown && status == 'add' && !isPrefill && !window.hideHint){
            // this.showSkipHint();
        }
        if(status == 'add'){
   ...
   /*  showOrHideUI(status){
    ...
    updateCrate(toAdd){
        ...
        setTimeout(()=>{
            ...
            this.crateCreatedTxt.setText(`${Global.filledTotal}/${window.rack1Visible?'7':'3.5'}`);
    ...
    cancelChangeAddress(){
        // Global.popupActive= false;
        Global.emitter.emit('popup_update', false);
        document.querySelector("#change_address_confirm").classList.remove("active")
    }
    doChangeAddress(){
        // Global.popupActive= false;
        Global.emitter.emit('popup_update', false);
    ...
    onResize(){
        ...
        this.BG.fillRect(this.extraLeftPer, this.extraTop, (this.c_w) * (Global.lastOrientation=='portrait'?0.5:0.2)- this.extraLeftPer/2, (Global.lastOrientation=='portrait'?400:300) * this.scaleFact);
        this.BG.fillStyle(0x55A383, 1)
        this.BG.fillRect((this.c_w) * (Global.lastOrientation=='portrait'?0.5:0.2)+ this.extraLeftPer/2, this.extraTop, (this.c_w) * (Global.lastOrientation=='portrait'?0.5:0.8)-this.extraLeftPer, (Global.lastOrientation=='portrait'?400:300) * this.scaleFact);
        this.add(this.BG);

        this.logo.setPosition(this.extraLeftPer*.75 + (this.c_w ) * (Global.lastOrientation=='portrait'?.25:0.1), (Global.lastOrientation=='portrait'?200:150) * this.scaleFact+this.extraTop)
        .setScale(this.scaleFact * (Global.lastOrientation=='portrait'?2:1.2));

        this.crateCreatedBG.setPosition(/* this.extraLeftPer + */ (this.c_w ) * (Global.lastOrientation=='portrait'?0.52:0.2)+ this.extraLeftPer/2 + 200 * this.scaleFact, (Global.lastOrientation=='portrait'?200:150) * this.scaleFact+this.extraTop)
        .setScale(this.scaleFact * (Global.lastOrientation=='portrait'?2.25:1.5));
        ...
        .setPosition(this.crateCreatedBG.x - (Global.lastOrientation=='portrait'?75:50) * this.scaleFact, this.crateCreatedBG.y + 5 * this.scaleFact)
        .setFontSize((Global.lastOrientation=='portrait'?65:45) * this.scaleFact);
        ...
        this.skipBtn.setPosition(this.c_w-this.extraLeftPer - ((window.isLoggedIn )?(Object.keys(window.address).length>1?(1200):750):300)*this.scaleFact, 150 * this.scaleFact+this.extraTop)
        .setScale(this.scaleFact * 0.75)

        this.changeAddress && this.changeAddress.setPosition(this.c_w-this.extraLeftPer - (750)*this.scaleFact, 150 * this.scaleFact+this.extraTop)
        .setScale(this.scaleFact * 0.75)
        ...
        if(Global.lastOrientation=='portrait'){
            ...
            .setPosition(this.c_w-this.extraLeftPer-200*this.scaleFact, 200*this.scaleFact+this.extraTop)
        }else{
        ...
        if(this.skipHintBGActive && Global.lastOrientation !=='portrait'){
            ...
            this.skipHintBG && this.skipHintBG
            .setScale(this.scaleFact*1.15);
        ...
        this.skipHintBG && this.skipHintBG
        .setPosition(this.skipBtn.x-330*this.scaleFact, this.skipBtn.y+340*this.scaleFact);

       

        this.skipHintTxt && this.skipHintTxt
        .setPosition(this.skipHintBG.x-475*this.scaleFact, this.skipHintBG.y-50*this.scaleFact)
        .setFontSize(55 * this.scaleFact)
        .setWordWrapWidth(1000*this.scaleFact)

        this.toggleSwitchLabel && this.toggleSwitchLabel
        .setPosition((Global.lastOrientation=='portrait'?((this.c_w ) * 0.5+ this.extraLeftPer/2 + 600 * this.scaleFact):this.crateCreatedBG.x + 650 * this.scaleFact), this.crateCreatedBG.y + (Global.lastOrientation=='portrait'?-75:5) * this.scaleFact)
        .setFontSize(55 * this.scaleFact)
        ...
        .setPosition(Global.lastOrientation=='portrait'?this.toggleSwitchLabel.x+this.toggleSwitchLabel.width*this.toggleSwitchLabel.scaleX*0.5:(this.crateCreatedBG.x + (1100) * this.scaleFact), (Global.lastOrientation=='portrait'?280:150) * this.scaleFact+this.extraTop)
        .setDisplaySize((Global.lastOrientation=='portrait'?300:200)*this.scaleFact, (Global.lastOrientation=='portrait'?300:200)*this.scaleFact);

        this.hideCheck && this.hideCheck
        .setPosition(this.skipHintBG.x-475*this.scaleFact, this.skipHintBG.y+150*this.scaleFact)

        this.skipHintBGActive && this.hideCheck && this.hideCheck
        .setScale(this.scaleFact * 0.8)

        this.hideBtn && this.hideBtn
        .setPosition(this.skipHintBG.x+475*this.scaleFact, this.skipHintBG.y+150*this.scaleFact)

        this.skipHintBGActive && this.hideBtn && this.hideBtn
        .setScale(this.scaleFact * 1)

        this.toggleInfoIcon && this.toggleInfoIcon
        .setPosition(Global.lastOrientation=='portrait'?this.toggleSwitchLabel.x+this.toggleSwitchLabel.width*this.toggleSwitchLabel.scaleX*1+20*this.scaleFact:(this.crateCreatedBG.x + (980) * this.scaleFact), this.crateCreatedBG.y - (50) * this.scaleFact)
        .setScale(this.scaleFact*(Global.lastOrientation=='portrait'?0.85:0.6))



        /* this.skipHintBGActive && */ this.hideInfo && this.hideInfo
