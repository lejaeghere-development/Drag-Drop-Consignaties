import { getLatestConfig, updateData } from './api';
...
export default class Score extends Phaser.GameObjects.Group {
  ...
  async showScore() {
    ...
    this.scoreHead = this.scene.make
      .text({
        x:
          Global.lastOrientation == 'portrait'
            ? this.c_w * 0.5
            : this.extraLeftPer + 1100 * this.scaleFact + this.extraTop / 2,
        y: this.c_h * 0.5 + 200 * this.scaleFact,
        text: `Top!`,
        origin: {
          x: 0.5,
          y: 0.5,
        ...
        style: {
          font:
            Global.lastOrientation == 'portrait'
              ? '' + String(240 * this.scaleFact) + 'px greycliff-bold'
              : '' + String(140 * this.scaleFact) + 'px greycliff-bold',
          fill: '#007892',
          align: 'center',
        },
      })
    ...
    this.scoreTxt = this.scene.make
      .text({
        ...
        y:
          this.c_h * 0.5 +
          (Global.lastOrientation == 'portrait' ? 600 : 450) * this.scaleFact,
        ...
        style: {
          font:
            Global.lastOrientation == 'portrait'
              ? '' + String(100 * this.scaleFact) + 'px greycliff-medium'
              : '' + String(60 * this.scaleFact) + 'px greycliff-medium',
          fill: '#007892',
          align: 'center',
          wordWrap: {
            width:
              (Global.lastOrientation == 'portrait' ? 2000 : 1400) *
              this.scaleFact,
          },
        },
      })
    ...
    this.replayBtn = this.create(
      this.scoreTxt.x,
      this.c_h * 0.5 +
        (Global.lastOrientation == 'portrait' ? 1100 : 750) * this.scaleFact,
      'items',
      'replay0000'
    )
      .setScale(
  ...
  onResize() {
      ...
      this.scoreHead
        .setPosition(
          Global.lastOrientation == 'portrait'
            ? this.c_w * 0.5
            : this.extraLeftPer + 1100 * this.scaleFact + this.extraTop / 2,
          this.c_h * 0.5 + 200 * this.scaleFact
        )
        .setFontSize(
          (Global.lastOrientation == 'portrait' ? 240 : 140) * this.scaleFact
        );
    ...
    this.scoreTxt &&
      this.scoreTxt
        ...
        .setWordWrapWidth(
          (Global.lastOrientation == 'portrait' ? 2000 : 1400) * this.scaleFact
        )
        .setFontSize(
          (Global.lastOrientation == 'portrait' ? 100 : 60) * this.scaleFact
        );
