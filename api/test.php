...
foreach ($results as $res) {
    if($res['admin'] != 1){
        ...
        foreach ($addressR as $resA) {
            ...
            $addressInfo[$res['email']]['addresses'][$resA['addressID']]= array();
            $addressInfo[$res['email']]['addresses'][$resA['addressID']]['title'] = $resA['title'];
            $addressInfo[$res['email']]['addresses'][$resA['addressID']]['street'] = $resA['street'];
            $addressInfo[$res['email']]['addresses'][$resA['addressID']]['housenumber'] = $resA['housenumber'];
            $addressInfo[$res['email']]['addresses'][$resA['addressID']]['postalcode'] = $resA['postalcode'];
            $addressInfo[$res['email']]['addresses'][$resA['addressID']]['city'] = $resA['city'];
        }
    }
