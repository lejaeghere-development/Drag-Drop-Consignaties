...
if($res){
 // create a new object
    ...
    $mail->Subject = 'DeliverYves: Wachtwoord wijziging';
    $mail->isHTML(TRUE);
           ...
           <tr>
              <td width='700' valign='top' align='center' style='background-repeat:no-repeat; background-size:cover;text-align: center;padding-bottom: 0px;width: 700px;padding-top: 60px;background-repeat: no-repeat;' class='full-width'>
                   ...
                   <table style='padding: 20px;  width: 700px;margin-bottom:100px;' width='700' cellpadding='0' cellspacing='0' border='0' class='full-width'>
                       ...
                       <tr>
                             <td class='txt_regular black_txt sub' style='padding-bottom: 28px;'>
                             Klik op onderstaande URL om uw wachtwoord opnieuw in te stellen
                            </td>
                        <tr>
                                  <td class='txt_regular black_txt sub'>
                                  <a href=".$domainBase."recoverPassword.php?recoveryCode=".$code." target='_blank'>".$domainBase."recoverPassword.php?recoveryCode=".$code."</a>
                                  </td>
                        </tr>
    ...
    if(!$mail->send()){
          echo base64_encode(json_encode(array('info' => 'Message could not be sent.')));
     //    echo base64_encode('Mailer Error: ' . $mail->ErrorInfo);
    } else {
        $data= array('tempCode'=> (string)$code);
        $condition= array('uid' => $res['uid']);
        $res2=$obj->updateData('deliveryves_count', $data, $condition);

          echo base64_encode(json_encode(array('message' => 'Success', 'info' => 'Message has been sent')));
    }
}else{
